...
class SupabaseAlertRepository:
    ...
    def claim_due_alerts_excluding_markets(self, worker_id: UUID, limit: int, excluded_markets: Sequence[str]) -> Sequence[ClaimedAlert]:
        return self._parse_alerts(self._request(
            "claim_due_alerts_excluding_markets",
    ...
    def release_alert(self, alert_id: UUID, worker_id: UUID, next_check_at: datetime) -> bool:
        data = self._request("release_alert", {"p_alert_id": str(alert_id), "p_worker_id": str(worker_id), "p_next_check_at": next_check_at.isoformat()})
        return bool(data)
    ...
    def record_alert_run(self, *, worker_id: UUID, alert_id: UUID, ticker: str, price: Decimal | None,
        ...
        data = self._request("record_alert_run", {
            "p_worker_id": str(worker_id), "p_alert_id": str(alert_id), "p_ticker": ticker,
            "p_price": str(price) if price is not None else None,
            "p_price_timestamp": price_timestamp.isoformat() if price_timestamp else None,
            "p_provider": provider, "p_trigger_hit": trigger_hit, "p_error_code": error_code,
            "p_duration_ms": duration_ms,
        })
        row = data[0] if isinstance(data, list) and data else data
        return int(row.get("retry_count", 0)) if isinstance(row, dict) else 0

    def ensure_market_session(self, market: str, session_date: date, opened_at: datetime, refresh_due_at: datetime) -> None:
        self._request("ensure_market_session", {
            "p_market": market, "p_session_date": session_date.isoformat(),
    ...
    def claim_session_refresh(self, market: str, session_date: date, worker_id: UUID) -> ClaimedSession | None:
        data = self._request("claim_session_refresh", {
            "p_market": market, "p_session_date": session_date.isoformat(), "p_worker_id": str(worker_id),
        })
        rows = data if isinstance(data, list) else []
        if not rows:
            return None
    ...
    def claim_session_refresh_items(self, session_id: UUID, worker_id: UUID, limit: int) -> Sequence[RefreshItem]:
        ...
        return tuple(RefreshItem(
            id=UUID(row["id"]), ticker=row["ticker"], market=row["market"], priority_class=row["priority_class"],
            status=row["status"], retry_count=int(row.get("retry_count", 0)),
            entry_min=self._decimal(row.get("entry_min")), entry_max=self._decimal(row.get("entry_max")),
            max_buy=self._decimal(row.get("max_buy")),
        ) for row in rows)
    ...
    def update_session_refresh_item(self, session_id: UUID, worker_id: UUID, ticker: str, **fields) -> bool:
        ...
        mapping = {
            "status": "p_status", "current_price": "p_current_price", "price_timestamp": "p_price_timestamp",
            "open_price": "p_open_price", "previous_close": "p_previous_close", "high_price": "p_high_price",
            "low_price": "p_low_price", "volume": "p_volume", "provider": "p_provider",
            "data_quality": "p_data_quality", "gap_pct": "p_gap_pct", "gap_flags": "p_gap_flags",
    ...
    def apply_session_price_to_alerts(self, session_id: UUID, worker_id: UUID, ticker: str, price: MarketPrice, next_check: datetime) -> int:
        return int(self._request("apply_session_price_to_alerts", {
            "p_session_id": str(session_id), "p_worker_id": str(worker_id), "p_ticker": ticker,
            "p_price": str(price.price), "p_price_timestamp": price.timestamp.isoformat(),
            "p_provider": price.provider, "p_next_check_at": next_check.isoformat(),
        }) or 0)
    ...
    def reserve_provider_credits(self, provider: str, requested: int, per_minute_limit: int, daily_budget: int) -> int:
        return int(self._request("reserve_provider_credits", {
            "p_provider": provider, "p_requested": int(requested),
            "p_per_minute_limit": int(per_minute_limit), "p_daily_budget": int(daily_budget),
        }) or 0)
